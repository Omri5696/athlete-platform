...
## שלב 1.6 — מדדים מלאים + מנוע ניתוח (2026-09-10) ✅

- [x] כל מדדי גרמין ב-`daily_metrics` (0003): צעדים, סטרס, נשימה, SpO2, שלבי שינה,
  ACWR, Garmin Readiness, VO2max, שעות שינה/קימה, משקל, ...
- [x] `Athlete` שוּנה ל-`days: DayMetrics[]` (עד 70 יום); `athletes.ts` בונה מ-DB
- [x] `src/lib/analysis.ts` — שכבות 1–2 מ-ANALYSIS.md: בייסליינים אישיים, סטיות,
  `daysOutsideBand`, slope; ציוני תחום (התאוששות / עומס / תחושה), פער קוהרנטיות,
  drivers. `readinessScore` נבנה מחדש על זה (החליף את הנוסחה הגסה)
- [x] `flags.ts` נכתב מחדש על המנוע (+ דגלי סטרס, נשימה, קוהרנטיות, ACWR)
- [x] הזנת נתונים ידנית — טופס בדף המתאמן (`LogMetrics`, `logDay` action)
- [x] דף מתאמן: פאנל "תמונת מצב" (ציוני תחום + drivers), 11 כרטיסי מגמה עם החלקה
- [x] seed מייצר 56 ימי היסטוריה לכל מתאמן מ-"סיפור" (`demo-data.ts`)

## שלב 2 — לולאת הקשר

- [ ] יומן מאמן לכל מתאמן (ציר זמן של הערות) — מזין הקשר ל-AI
